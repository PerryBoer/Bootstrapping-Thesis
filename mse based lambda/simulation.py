...
class SimulationRunner:
    def __init__(
        ...
        signal_type: str,
        error_type: str,
        level: float = 0.90, # confidence level for bootstrap CI
        tracked_indices: List[int] = [5, 20], # for validation and testing purposes, and eventually for plotting 
        subdir: str = "results/plots" #  to save results for every configruation 
    ):
        self.method = method
    ...
    def compute_lambda_grid(self, X: np.ndarray, y: np.ndarray, n_grid: int = 50) -> np.ndarray:
        # Standardize X and y before computing lambda_max
        X_scaled = StandardScaler().fit_transform(X)
        y_scaled = (y - y.mean()) / y.std()

        lambda_max = np.max(np.abs(X_scaled.T @ y_scaled)) / len(y)
        lambda_min = lambda_max * 1e-3
        return np.logspace(np.log10(lambda_max), np.log10(lambda_min), n_grid)[::-1]
    ...
    def run(self) -> Dict[str, Any]:
        for r in range(self.R):
            ...
            lambda_grid = self.compute_lambda_grid(X, y)
