...
class DGP:
    def __init__(
        ...
        rho_cov_X: float = 0.0,  # AR(1)-like covariance structure for X
        fixed_X: Optional[np.ndarray] = None,  # optional user-provided fixed X
        seed: int = 42
    ):
        self.n = n
        self.p = p
    ...
    def _generate_beta(self):
        ...
        if self.signal_type == "strong":
            ...
            values = np.random.uniform(1, 2, self.s) * np.random.choice([-1, 1], size=self.s)
        else:
    ...
    def _generate_X(self):
        ...
        return np.random.multivariate_normal(mean=np.zeros(self.p), cov=cov, size=self.n)

    def _generate_errors(self, X):
        u = np.random.normal(0, 1, size=self.n)
    ...
    def generate(self):
        ...
        y = X @ beta + errors
        support = np.flatnonzero(beta)  # indices of non-zero beta
        return X, y, beta, errors, support


# Example usage:

# dgp = DGP(n=200, p=100, s=10, signal_type="strong", error_type="heteroskedastic", rho_cov_X=0.2)
# X, y, beta, eps, support = dgp.generate()
...
# print("First 5 values of errors:", eps[:5])
